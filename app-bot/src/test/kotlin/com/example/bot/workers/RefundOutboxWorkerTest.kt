...
class RefundOutboxWorkerTest {
    ...
    @Test
    fun `success flow marks message as sent`() =
        runTest {
    ...
    @Test
    fun `429 response schedules retry using header`() =
        runTest {
            val outbox = mockk<OutboxRepository>()
            val metrics = MetricsProvider(MetricsProvider.simpleRegistry())
            val message = sampleMessage()
            ...
            coEvery { outbox.pickBatchForTopics(any(), any()) } returns listOf(message)
            coEvery {
                outbox.markFailedWithRetry(message.id, any(), capture(nextAttempt))
            } returns BookingCoreResult.Success(message)
            val worker =
    ...
    @Test
    fun `5xx without retry-after uses backoff`() =
        runTest {
            val outbox = mockk<OutboxRepository>()
            val metrics = MetricsProvider(MetricsProvider.simpleRegistry())
            val message = sampleMessage()
    ...
    @Test
    fun `4xx response marks permanent failure`() =
        runTest {
            val outbox = mockk<OutboxRepository>()
            val metrics = MetricsProvider(MetricsProvider.simpleRegistry())
            val message = sampleMessage()
    ...
    @Test
    fun `koin wiring delivers worker`() =
        runTest {
            val outbox = mockk<OutboxRepository>()
            val metrics = MetricsProvider(MetricsProvider.simpleRegistry())
            val message = sampleMessage()
