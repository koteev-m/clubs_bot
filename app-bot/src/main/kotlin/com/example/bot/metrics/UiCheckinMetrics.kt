...
object UiCheckinMetrics {
    ...
    @PublishedApi @Volatile internal var byNameTimer: Timer? = null
    ...
    fun bind(registry: MeterRegistry) {
        ...
        cLateOverride =
            registry.find("ui.arrival.late_override_total").counter()
                ?: Counter.builder("ui.arrival.late_override_total")
                    .description("Arrivals outside arrival_window with override (e.g., CALLED)")
                    .register(registry)
    }

    fun incTotal() { cScanTotal?.increment() }
    fun incError() { cScanError?.increment() }
    ...
    fun incByNameTotal() { cByNameTotal?.increment() }
    fun incByNameError() { cByNameError?.increment() }
    ...
    fun incLateOverride() { cLateOverride?.increment() }
}
