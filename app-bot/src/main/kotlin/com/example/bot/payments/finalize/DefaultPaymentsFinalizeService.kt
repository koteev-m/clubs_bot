...
class DefaultPaymentsFinalizeService(
    ...
    override suspend fun finalize(
        ...
        val paymentStatus =
            when (bookingResult) {
                ...
                BookingCmdResult.IdempotencyConflict -> throw PaymentsFinalizeService.ConflictException(
                    "booking idempotency conflict",
                )
                BookingCmdResult.DuplicateActiveBooking -> throw PaymentsFinalizeService.ConflictException(
    ...
    private suspend fun tryPersistResult(
        ...
        val created =
            paymentsRepository.createInitiated(
                bookingId = bookingId,
        ...
        paymentsRepository.findByIdempotencyKey(idemKey)
            ?: created.copy(
                status = paymentStatus,
                externalId = tokenDigest,
            )
    ...
    private fun statusFor(tokenDigest: String?): String =
        if (tokenDigest != null) STATUS_CAPTURED else STATUS_NO_PAYMENT

    private fun hashToken(token: String): String {
        val digest = MessageDigest.getInstance("SHA-256")
        val hashed = digest.digest(token.toByteArray(StandardCharsets.UTF_8))
        return hashed.joinToString(separator = "") { b ->
    ...
    companion object {
        ...
        // detekt:MagicNumber — выносим маску в константу
