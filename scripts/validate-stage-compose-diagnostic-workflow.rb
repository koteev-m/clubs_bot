...
module StageComposeDiagnosticWorkflow
  PATH = ".github/workflows/stage-compose-diagnostic.yml"
  ...
  def self.validate(policy, workflow)
    ...
    validate = {"name" => "Validate fixed request before credentials", "shell" => "bash",
                "env" => env, "run" => "python3 -I -S -B scripts/deploy/stage-compose-diagnostic.py --validate"}
    expected = {
      "name" => "Stage Compose Diagnostics (CLB-91 manual)",
      "on" => {"workflow_dispatch" => {"inputs" => {"confirmation" => {
        "description" => "Type CLB-91:35206468948:diagnose-compose-subset after separate diagnostic authorization",
      ...
      "jobs" => {
        ...
        "diagnose" => {"needs" => "validate", "runs-on" => "ubuntu-latest", "timeout-minutes" => 5,
          ...
          {"name" => "Collect fixed read-only Compose diagnostics", "shell" => "bash",
           "run" => "python3 -I -S -B scripts/deploy/stage-compose-diagnostic.py",
    ...
    policy.validate_exact_value(workflow, expected, "#{PATH}: fixed read-only diagnostic capability")
  end
end
