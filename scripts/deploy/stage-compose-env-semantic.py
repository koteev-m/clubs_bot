...
if __name__ == '__main__' and not (sys.flags.isolated and sys.flags.no_site):
    raise SystemExit('compose-env-semantic:v=1 result=unavailable reason=request')
...
REMOTE_PATH = 'scripts/deploy/stage-compose-env-semantic-operation.py'
RUNNER_PATH = 'scripts/deploy/stage-compose-env-semantic.py'
WORKFLOW_PATH = '.github/workflows/stage-compose-env-semantic.yml'
...
LOCAL_SOURCES = (RUNNER_PATH, 'scripts/deploy/corrected-stage-release.py',
                 ...
                 'scripts/deploy/stage-compose-diagnostic-operation.py',
                 'scripts/deploy/stage-compose-env-file-plan.py',
                 'scripts/deploy/stage-compose-env-semantic-runtime.json')
REMOTE_SOURCES = ('scripts/deploy/stage-compose-diagnostic-operation.py',
                  'scripts/deploy/stage-compose-env-file-plan.py',
                  'scripts/deploy/release_private_root.py', REMOTE_PATH,
                  'scripts/deploy/stage-compose-env-semantic-runtime.json')
...
SOURCE_PINS = {
    ...
    'scripts/deploy/stage-compose-diagnostic-operation.py': '2487c9b9d82de02ab2df9d11bd54c45c75e423d595079c9cf3ab2e10890ee96c',
}
...
try:
    ...
    frame = b'clb91-semantic-auth:v=1 tag=' + tag + b' ' + body
...
def parse_result(raw, code, nonce, protocol):
    ...
    match = re.fullmatch(rb'clb91-semantic-auth:v=1 tag=([0-9a-f]{64}) (compose-env-semantic:v=1 [^\r\n]*\n)', raw)
    require(match is not None)
...
def main(env, args, cancelled):
    ...
    try:
        ...
        if args == ['--validate']:
            return 'compose-env-semantic-validation:v=1 result=ok', 0
        validate_target(env)
        ...
        return 'compose-env-semantic:v=1 result=unavailable reason=' + reason, 1
...
def publish(line, code, cancelled):
    ...
    if cancelled[0] or set(signal.sigpending()).intersection(WATCHED):
        line, code = 'compose-env-semantic:v=1 result=unavailable reason=interrupted', 1
    body = (line + '\n').encode('ascii')
