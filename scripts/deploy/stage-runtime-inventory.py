...
if __name__ == '__main__' and not (sys.flags.isolated and sys.flags.no_site):
    raise SystemExit('runtime-inventory:v=1 {"reason":"request","result":"unavailable"}')
...
REMOTE_PATH = 'scripts/deploy/stage-runtime-inventory-operation.py'
RUNNER_PATH = 'scripts/deploy/stage-runtime-inventory.py'
WORKFLOW_PATH = '.github/workflows/stage-runtime-inventory.yml'
CONFIRMATION = 'CLB-91:35604124263:inventory-stage-runtime'
SOURCE_LIMIT = 65536
FRAME_LIMIT = 12288
...
REMOTE_SOURCES = (REMOTE_PATH, REFERENCE_PATH)
...
SOURCE_PINS = {
    ...
    REFERENCE_PATH: 'd2c7794fa073ab310df46a80880a83fceb0a2eb450aae410e123266f07c83903',
}
...
try:
    ...
    names = ('stage-runtime-inventory-operation.py', 'stage-compose-env-semantic-runtime.json')
    ...
    frame = b'clb91-runtime-inventory-auth:v=1 tag=' + tag + b' ' + body
    assert len(frame) <= 12288 and os.write(1, frame) == len(frame)
...
def parse_result(raw, code, nonce, protocol):
    ...
    match = re.fullmatch(rb'clb91-runtime-inventory-auth:v=1 tag=([0-9a-f]{64}) (runtime-inventory:v=1 [^\r\n]*\n)', raw)
    require(match is not None)
...
def main(env, args, cancelled):
    ...
    try:
        ...
        if args == ['--validate']:
            return 'runtime-inventory-validation:v=1 result=ok', 0
        validate_target(env)
        ...
        return 'runtime-inventory:v=1 '+json.dumps(dict(reason=reason,result='unavailable'),sort_keys=True,separators=(',',':')), 1
...
def publish(line, code, cancelled):
    ...
    if cancelled[0] or set(signal.sigpending()).intersection(WATCHED):
        line, code = 'runtime-inventory:v=1 {"reason":"interrupted","result":"unavailable"}', 1
    body = (line + '\n').encode('ascii')
