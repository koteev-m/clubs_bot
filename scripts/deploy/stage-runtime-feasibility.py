...
if __name__ == '__main__' and not (sys.flags.isolated and sys.flags.no_site):
    raise SystemExit('runtime-feasibility:v=1 {"reason":"request","result":"unavailable"}')
...
REMOTE_PATH = 'scripts/deploy/stage-runtime-feasibility-operation.py'
RUNNER_PATH = 'scripts/deploy/stage-runtime-feasibility.py'
WORKFLOW_PATH = '.github/workflows/stage-runtime-feasibility.yml'
CONFIRMATION = 'CLB-91:35646675380:runtime-feasibility'
SOURCE_LIMIT = 65536
FRAME_LIMIT = 131200
WATCHED = (signal.SIGHUP, signal.SIGINT, signal.SIGTERM)
REFERENCE_PATH = 'scripts/deploy/stage-compose-env-semantic-runtime.json'
INVENTORY_PATH = 'scripts/deploy/stage-runtime-inventory-operation.py'
INPUTS_PATH = 'scripts/tests/linux-semantic/amd64-inputs.json'
PACKAGES_PATH = 'scripts/tests/linux-semantic/amd64-packages.txt'
LOCAL_SOURCES = (RUNNER_PATH, INVENTORY_PATH, INPUTS_PATH, PACKAGES_PATH, 'scripts/deploy/corrected-stage-release.py',
                 'scripts/deploy/release_private_root.py', 'scripts/deploy/release_authority.py', REFERENCE_PATH)
REMOTE_SOURCES = (REMOTE_PATH, REFERENCE_PATH, INVENTORY_PATH, INPUTS_PATH, PACKAGES_PATH)
...
SOURCE_PINS = {
    INVENTORY_PATH: '508b243b77a636d79366c45471470cb7d3b4974bb555ac8370059da1e866f002',
    INPUTS_PATH: 'a365d37e6bf45dacfe672364d2e3493bc3c786e692940e4ee0566b8f97fd5833',
    PACKAGES_PATH: '17ac090efb6ba58f50f72f7e2e7425e6db5941b7a5758b4099e4553058b6983c',
    ...
    REFERENCE_PATH: '93a9d29cba93770fab9cc6605709a3b159cfb7ce2c627677ff77bd9cacd62008',
}
...
try:
    ...
    names = ('stage-runtime-feasibility-operation.py', 'stage-compose-env-semantic-runtime.json',
             'stage-runtime-inventory-operation.py', 'amd64-inputs.json', 'amd64-packages.txt')
    ...
    reference = json.loads(raw[names[1]], object_pairs_hook=unique)
    primitive_code = compile(raw[names[2]], '/clb91-source/scripts/deploy/'+names[2], 'exec')
    inputs = json.loads(raw[names[3]], object_pairs_hook=unique)
    package_text = raw[names[4]].decode('ascii')
    compiled = compile(raw[names[0]], '/clb91-source/scripts/deploy/'+names[0], 'exec')
    assert not cancelled[0]
    module = types.ModuleType('fixed_runtime_inventory')
    module.__file__ = '/clb91-source/scripts/deploy/'+names[0]
    primitive = types.ModuleType('fixed_inventory_primitives')
    primitive.__file__ = '/clb91-source/scripts/deploy/'+names[2]
    exec(primitive_code, primitive.__dict__)
    module.I = primitive
    module.REFERENCE, module.INPUTS, module.PACKAGE_TEXT = reference, inputs, package_text
    exec(compiled, module.__dict__)
    body = module.collect(lambda: cancelled[0])
    ...
    frame = b'clb91-runtime-feasibility-auth:v=1 tag=' + tag + b' ' + body
    assert len(frame) <= 131200 and os.write(1, frame) == len(frame)
...
def main(env, args, cancelled):
    ...
    try:
        ...
        if args == ['--validate']:
            return 'runtime-feasibility-validation:v=1 result=ok', 0
        validate_target(env)
        protocol = types.ModuleType('fixed_diagnostic_protocol')
        primitive = types.ModuleType('fixed_inventory_primitives')
        primitive_code = compile(sources[INVENTORY_PATH], '<fixed-inventory-primitives>', 'exec')
        protocol_code = compile(source, '<fixed-feasibility-protocol>', 'exec')
        exec(primitive_code, primitive.__dict__)
        protocol.I = primitive
        protocol.REFERENCE = json.loads(sources[REFERENCE_PATH])
        protocol.INPUTS = json.loads(sources[INPUTS_PATH])
        protocol.PACKAGE_TEXT = sources[PACKAGES_PATH].decode('ascii')
        exec(protocol_code, protocol.__dict__)
        ...
        require(len(bundle) <= 524288)
        control = json.dumps(dict(principal=env['SSH_USER'], sha256=hashlib.sha256(bundle).hexdigest()),
                             separators=(',', ':')).encode()
        ...
        with transport.pinned_hosts(env) as (fd, reference):
            ...
            captured = transport.capture_result(ssh_argv(env, reference), payload, env=child_env,
                                                 timeout=110, limit=FRAME_LIMIT, pass_fds=(fd,))
        ...
        return 'runtime-feasibility:v=1 '+json.dumps(dict(reason=reason,result='unavailable'),sort_keys=True,separators=(',',':')), 1
...
def publish(line, code, cancelled):
    ...
    if cancelled[0] or set(signal.sigpending()).intersection(WATCHED):
        line, code = 'runtime-feasibility:v=1 {"reason":"interrupted","result":"unavailable"}', 1
    body = (line + '\n').encode('ascii')
    require(len(body) <= 131072)
