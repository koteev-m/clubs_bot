...
module StageComposeModeWorkflow
  PATH = ".github/workflows/stage-compose-mode-repair.yml"
  ...
  def self.validate(policy, workflow)
    ...
    validate = {"name" => "Validate fixed request before credentials", "shell" => "bash",
                "env" => env, "run" => "python3 -I -S -B scripts/deploy/stage-compose-mode-repair.py --validate"}
    expected = {
      "name" => "Stage Compose Mode Repair (CLB-91 manual)",
      "on" => {"workflow_dispatch" => {"inputs" => {"confirmation" => {
        "description" => "Type CLB-91:35108589661:repair-compose-mode-0600 after separate repair authorization",
      ...
      "jobs" => {
        ...
        "repair" => {"needs" => "validate", "runs-on" => "ubuntu-latest", "timeout-minutes" => 5,
          ...
          {"name" => "Repair only the fixed Compose file mode", "shell" => "bash",
           "run" => "python3 -I -S -B scripts/deploy/stage-compose-mode-repair.py",
    ...
    policy.validate_exact_value(workflow, expected, "#{PATH}: fixed mode-repair capability")
  end
end
