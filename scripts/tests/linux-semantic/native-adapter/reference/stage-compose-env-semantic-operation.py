...
D = P = S = MANIFEST = None
# Fixed secondary classes only; primary refusal stays independently reportable.
LAST_CLEANUP_ERRORS = []
...
# Trigger Psych lazy encoding loads with fixed public data before private capture.
Psych.parse_stream('services: {app: {image: fixture:local}}')
...
def diagnose(principal, cancelled):
    ...
    try:
        ...
        for label, owned in (('capture_close', context), ('runtime_close', runtime)):
            if owned is not None:
                try:
                    owned.close()
                except BaseException:
                    LAST_CLEANUP_ERRORS.append(label)
                    if b' result=equivalent ' in body:
