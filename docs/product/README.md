...
Decision log содержит 38 records: 18 `ACCEPTED_DECISION` и 20 `DECISION_REQUIRED`. 16 records, принятых до `DEC-037`, сохраняются без изменений (`DEC-003`, `DEC-004`, `DEC-005`, `DEC-007`, `DEC-017`, `DEC-025`, `DEC-026`, `DEC-028`–`DEC-036`), а `DEC-001`, `DEC-002`, `DEC-006` и `DEC-027` остаются pending. `DEC-037` — seventeenth accepted record и отдельное operational security/governance decision; `DEC-038` — eighteenth accepted record и отдельное operational security/governance decision, узко заменяющее provider/VNC host-authentication prerequisite только для exact stage endpoint/Ed25519 fingerprint при сохранении strict checking и всех остальных execution gates. Neither decision is a product capability, does not enter the product overlay `PRODUCT_SPEC.md`, and does not change its 170 immutable source requirements. Рекомендации имеют отдельные dispositions/defaults и не становятся принятым выбором через wording, dependency или существование кода.
