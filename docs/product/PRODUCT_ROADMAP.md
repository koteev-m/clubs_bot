...
- The ordered gates below define dependency and authority boundaries, not a live backlog. Before each operation,
  classify every gate from current live evidence as `SATISFIED`, `UNSATISFIED` or `STALE`. A later gate may execute
  only when every earlier gate is `SATISFIED`. Continue at the first `UNSATISFIED` gate, never repeat a `SATISFIED`
  mutation or reuse consumed authorization, and stop for a separately scoped reconciliation task if any required
  evidence is `STALE`, conflicting or missing. The roadmap text itself grants no workflow, environment, secret,
  deployment, SSH or recovery authority.
