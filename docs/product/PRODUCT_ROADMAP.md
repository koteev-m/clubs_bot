...
- Implementation commit `cfb77b3a9df3291038f0b2792d2185af12bf2aa3` completes bounded `AS_BUILT` evidence for `PSL-AC-13`, the read/authorization/denial portions of `PSL-AC-16`–`PSL-AC-19`, and the read-only list/filter/detail/thread/denial portion of `PSL-AC-32`. Paired PostgreSQL/H2 V058 migrations create `permissions` and `user_role_permissions`; a grant is tied to one `user_roles.id`, cascades with that assignment, rejects duplicates and has no default grant. The catalog provides `support.view`, `support.reply` and `support.status.manage`. Every staff operation requires one resolved application user, `MANAGER` or `CLUB_ADMIN`, matching `CLUB` scope and exact attached permission; role, scope or permission alone grants nothing. `GET /api/support/staff/clubs`, permission-gated list/filter and scoped detail/thread provide deterministic minimal reads with non-enumerating foreign/missing detail. `/app?mode=support` is a bounded GET-only React surface with club selector, filter, list, detail/thread and denial-safe cache clearing; it has no mutation controls, permission UI or canonical-shell redesign. H2/PostgreSQL/Ktor/frontend evidence and full quality gates passed. This implementation has no invented PR/merge provenance and does not complete reply, status/lifecycle, audit, truthful delivery, restart or staging acceptance.
...
- **Next bounded area: permission-gated staff mutations and accepted lifecycle core.** Begin with `PSL-AC-14`, `PSL-AC-15`, `PSL-AC-20`–`PSL-AC-22`, the mutation portions of `PSL-AC-16`–`PSL-AC-19`, and the mutation portion of `PSL-AC-32`. Reuse the delivered exact permission foundation and read boundary; do not redesign them here.
- explicit reply/take/status behavior must preserve independent `support.reply` and `support.status.manage` permissions, the matching CLUB assignment boundary and all foreign-data denials; `support.view` remains read-only;
- exact `DEC-025` lifecycle `NEW → IN_PROGRESS → RESOLVED → CLOSED`, accepted confirmation only for explicit resolve and terminal-state enforcement;
- required audit semantics must be preserved from the start of mutation work: actor/club/ticket/old/new/delivery outcome as applicable, with no message-body duplication;
- truthful observable Telegram delivery without false success;
- later resolve/close lifecycle, delivery, audit completion, broader restart evidence and end-to-end staging smoke with persistence, authorization, lifecycle, restart and delivery outcome.
...
`PASS` in this crosswalk means that the former item keeps its canonical acceptance mapping; it is not an implementation status. `PSL-AC-01`–`PSL-AC-13` have bounded `AS_BUILT` evidence.

`PSL-AC-16`–`PSL-AC-19` and `PSL-AC-32` retain `PARTIAL` overall status: only their read/authorization/denial portions are built, while mutation/lifecycle acceptance remains incomplete.
...
Technical implementation prerequisites remain: permission-gated reply/status mutations and accepted lifecycle core (`PSL-AC-14`, `PSL-AC-15`, `PSL-AC-20`–`PSL-AC-22`, mutation portions of `PSL-AC-16`–`PSL-AC-19` and `PSL-AC-32`); required audit semantics; later resolve/close, delivery, restart correctness; delivery queue/outbox/retry design; Ktor/Postgres tests for the remaining flow; successful staging smoke. Эти choices должны сохранять accepted contract и не меняют статус `ACCEPTED_NOT_IMPLEMENTED` сами по себе.
...
- **Dependencies:** completed private `/start` minimal identity provisioning, explicit production-backed club/category selection, bounded `PSL-AC-11`/ticket-creation `PSL-AC-36` evidence, owner-safe `PSL-AC-12` guest reads, and the delivered V058 exact-permission staff read boundary (`PSL-AC-13`, bounded read/denial `PSL-AC-16`–`PSL-AC-19`, read-only `PSL-AC-32`); accepted lifecycle/status transitions; required audit semantics; truthful delivery design.
