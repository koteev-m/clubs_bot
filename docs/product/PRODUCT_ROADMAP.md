...
- generic RBAC/club-scope primitives plus the delivered exact-assignment support permission and read boundary; legacy reply/status behavior still does not meet the accepted lifecycle/delivery contract;
...
- Current feature-branch implementation commit `6de082b93d6e9ada9a857b3e5d953f8d56581693` completes bounded `AS_BUILT` evidence for `PSL-AC-14`, the take portion of `PSL-AC-15`, `PSL-AC-20`–`PSL-AC-22`, the mutation/denial portions of `PSL-AC-16`–`PSL-AC-19`, the take/reply portion of `PSL-AC-32`, and bounded mutation/audit rollback evidence for `PSL-AC-36`. It has no invented PR, merge or documentation commit. The authoritative transaction locks the ticket and verifies exactly one acting-user `MANAGER`/`CLUB_ADMIN` CLUB assignment for the ticket club with the requested permission attached to that same `user_roles.id`; take requires `support.status.manage`, reply requires `support.reply`. Another assignment/user/club, `GLOBAL` scope or route precheck alone cannot grant authority. No accepted assignment with the required permission is `403 / support_ticket_forbidden`; permission elsewhere with foreign/missing ticket is identical `404 / support_ticket_not_found`; invalid lifecycle is `409 / invalid_state`; persistence/audit failure is generic `500 / internal_error`.
...
- `POST /api/support/tickets/{ticketId}/status` is deliberately disabled: after authentication, strict ID parsing, exact `support.status.manage` authorization and scoped ticket verification, every body returns `409 / invalid_state` without body parsing, `setStatus`, ticket mutation or audit. `/assign` and `/reply` remain permission-specific. The public reply result excludes `ownerUserId`; HTTP success means persisted reply, never Telegram delivery. `/app?mode=support` now uses server capabilities for take only on `NEW` plus `canTakeInWork`, and reply only on `NEW`/`IN_PROGRESS` plus `canReply`; it trims/limits reply text, prevents duplicate submit, refreshes after success, says «Ответ сохранён», clears/refreshes on `403` and clears selection on `404`. No resolve/close/reopen/status selector/delivery state/permission UI or canonical-shell redesign was added.
...
- **Next bounded area: remaining lifecycle core.** Implement the remaining `PSL-AC-15`, `PSL-AC-25`–`PSL-AC-28`, `PSL-AC-31`, remaining lifecycle portions of `PSL-AC-16`, `PSL-AC-19` and `PSL-AC-32`, bounded status/close audit work in `PSL-AC-30`, and associated failure evidence in `PSL-AC-36`.
- preserve the accepted `DEC-025` `NEW → IN_PROGRESS → RESOLVED → CLOSED` graph, explicit resolve confirmation and terminal-state enforcement; do not use the disabled generic status route as a bypass;
- extend the existing exact transactional permission/denial and privacy-safe audit boundary to remaining status/close mutations, without message-body duplication;
- keep truthful Telegram delivery as a separate later area, `PSL-AC-23` and `PSL-AC-24`; do not design it here;
- later restart correctness and end-to-end staging smoke remain required.
...
`PASS` in this crosswalk means that the former item keeps its canonical acceptance mapping; it is not an implementation status. `PSL-AC-01`–`PSL-AC-14` and `PSL-AC-20`–`PSL-AC-22` have bounded `AS_BUILT` evidence.

`PSL-AC-15`, `PSL-AC-16` and `PSL-AC-32` retain `PARTIAL` overall status. `PSL-AC-17`–`PSL-AC-19` are `AS_BUILT` from the combined read/mutation/denial evidence. The bounded mutation/audit rollback portion of `PSL-AC-36` is `AS_BUILT` evidence, not complete support audit acceptance.
...
Technical implementation prerequisites remain: remaining lifecycle core (`PSL-AC-15`, `PSL-AC-25`–`PSL-AC-28`, `PSL-AC-31`, remaining lifecycle portions of `PSL-AC-16`, `PSL-AC-19` and `PSL-AC-32`), status/close audit and failure evidence (`PSL-AC-30` and remaining `PSL-AC-36`), separate truthful delivery work (`PSL-AC-23`/`PSL-AC-24`), restart correctness, Ktor/Postgres evidence for the remaining flow and successful staging smoke. Эти choices должны сохранять accepted contract и не меняют статус `ACCEPTED_NOT_IMPLEMENTED` сами по себе.
...
- **Dependencies:** completed private `/start` minimal identity provisioning, explicit production-backed club/category selection, bounded `PSL-AC-11`/ticket-creation `PSL-AC-36` evidence, owner-safe `PSL-AC-12` guest reads, merged PR #485 exact-permission staff read boundary, and current bounded staff-mutation transaction/audit evidence (`PSL-AC-13`–`PSL-AC-14`, `PSL-AC-17`–`PSL-AC-22`, bounded portions of `PSL-AC-15`, `PSL-AC-16`, `PSL-AC-32` and `PSL-AC-36`); remaining accepted resolve/close/status transitions, required status/close audit semantics and truthful delivery design.
