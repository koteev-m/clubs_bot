...
- generic RBAC/club-scope primitives plus the delivered exact-assignment support permission, bounded staff surface and accepted lifecycle; truthful delivery remains separate;
...
- Immutable implementation commit `63084ac39224359601f978f7a9bebf4d22f5fc3d` completes the remaining accepted lifecycle evidence: V059 PostgreSQL/H2 preserves legacy rows while allowing exactly `new`, `opened`, `in_progress`, `answered`, `resolved`, `closed` and rejecting `waiting`; PostgreSQL migration fails closed on expected-constraint drift. Confirmed `POST /api/support/tickets/{ticketId}/resolve` permits only `IN_PROGRESS → RESOLVED`, owner guest message resumes only `RESOLVED → IN_PROGRESS`, and body-independent `POST /api/support/tickets/{ticketId}/close` permits only `RESOLVED → CLOSED`. `CLOSED` rejects guest message, take, reply, resolve, close and generic status mutation; a guest can still create a separate ticket. Exact `support.status.manage` authority is rechecked under lock for resolve/close. Resolve emits one status audit; close emits one `SUPPORT_CLOSE` and one status audit; guest resume emits one status audit. Status metadata is only old/new value; audit failure rolls back all mutation/message/audit writes. PostgreSQL evidence proves resolve/close races and reply/resolve plus guest/close serialization; H2, Ktor and Mini App evidence covers migration, errors, rollback, confirmation and server-derived controls. No PR, merge or documentation commit is claimed for `63084…`.
- `POST /api/support/tickets/{ticketId}/status` remains deliberately disabled: after authentication, strict ID parsing, exact `support.status.manage` authorization and scoped ticket verification, every body returns `409 / invalid_state` without body parsing, `setStatus`, ticket mutation or audit. `/assign`, `/reply`, `/resolve` and `/close` remain permission-specific. The public reply result excludes `ownerUserId`; HTTP success means persisted reply, never Telegram delivery. `/app?mode=support` uses server capabilities for take only on `NEW`, reply only on `NEW`/`IN_PROGRESS`, confirmed Resolve only on `IN_PROGRESS`, and Close only on `RESOLVED`; it prevents duplicate submit, reloads after success, says «Ответ сохранён», «Обращение решено» or «Обращение закрыто», clears/refreshes on `403` and clears selection on `404`. There is no reopen, generic status selector, delivery state, permission-management UI or canonical-shell redesign.
...
- **Next bounded area: truthful Telegram delivery.** Implement `PSL-AC-23` and `PSL-AC-24`, the delivery-result portion of `PSL-AC-30`, and the corresponding delivery failure/rollback/observability evidence; do not design the delivery architecture in this roadmap.
- delivery success/failure must be truthful: a persisted reply must not be represented as delivered without an observed result, and delivery-result audit must preserve the existing privacy boundary;
- after delivery, complete process-restart proof for `PSL-AC-29` and the accepted staging smoke remain required;
...
`PASS` in this crosswalk means that the former item keeps its canonical acceptance mapping; it is not an implementation status. `PSL-AC-01`–`PSL-AC-22`, `PSL-AC-25`–`PSL-AC-28`, `PSL-AC-31` and `PSL-AC-32` have bounded `AS_BUILT` evidence.

`PSL-AC-29` is `PARTIAL`: DB/fresh-repository evidence exists, but complete process-restart E2E is not claimed. `PSL-AC-30` is `PARTIAL` solely at delivery-result audit: reply/status/close audit evidence is `AS_BUILT`. Bounded provisioning, ticket/message and lifecycle/audit rollback evidence for `PSL-AC-36` is `AS_BUILT`; the full exact provisioning/ticket/message failure boundary is not raised here.
...
Technical implementation prerequisites remain: truthful delivery (`PSL-AC-23`/`PSL-AC-24`), delivery-result audit and corresponding failure/rollback/observability evidence (`PSL-AC-30`), then complete restart correctness (`PSL-AC-29`) and successful staging smoke. These choices preserve the accepted contract and do not change the complete-slice status `ACCEPTED_NOT_IMPLEMENTED` by themselves.
...
- **Dependencies:** completed private `/start` minimal identity provisioning, explicit production-backed club/category selection, bounded `PSL-AC-11`/ticket-creation `PSL-AC-36` evidence, owner-safe `PSL-AC-12` guest reads, merged PR #485 exact-permission staff read boundary, merged PR #486 take/reply evidence, and immutable `63084ac39224359601f978f7a9bebf4d22f5fc3d` accepted resolve/guest-resume/close lifecycle, status/close audit and rollback evidence. The next dependency is truthful Telegram delivery with delivery-result audit and failure observability; afterward complete restart proof and staging smoke remain.
