...
- **Next bounded area: complete process-restart E2E proof for `PSL-AC-29`.** Prove that provisioned identity, accepted ticket state, complete persisted thread and delivery/audit correlation remain available after a real process restart; do not design a restart harness in this roadmap.
- then execute the accepted staging smoke; it remains the only staging boundary and must include the already implemented truthful-delivery behavior;
...
Technical implementation prerequisites remain: complete restart correctness (`PSL-AC-29`) and successful staging smoke. Truthful delivery is complete; its deliberate absence of worker, queue/outbox processor and retry architecture is not a gap in this bounded accepted outcome. These choices preserve the accepted contract and do not change the complete-slice status `ACCEPTED_NOT_IMPLEMENTED` by themselves.
...
- **Dependencies:** completed private `/start` minimal identity provisioning, explicit production-backed club/category selection, `PSL-AC-11`/`PSL-AC-36` transaction-failure evidence, owner-safe `PSL-AC-12` guest reads, merged PR #485 exact-permission staff read boundary, merged PR #486 take/reply evidence, merged PR #487 accepted resolve/guest-resume/close lifecycle/status/close audit evidence, and immutable `fa62ef835348768c05b7ddf8ba224a6a8edfa805` truthful Telegram delivery/delivery-result audit/failure-observability evidence.
- **Remaining dependencies:** complete restart proof and staging smoke.
